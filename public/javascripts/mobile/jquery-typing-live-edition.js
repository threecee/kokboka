...
(function ($) {
    ...
    function listenToTyping(elem, options) {
        ...
        $elem.on('keypress', settings.target, startTyping);

        // listen to backspace and delete presses
        $elem.on('keydown', settings.target, function (event) {
        ...
        // listen to keyups
        $elem.on('keyup', settings.target, stopTyping);

        // listen to blurs
        $elem.on('focusout', settings.target, function (event) {
            stopTyping(event, 0);
        });
    }
})(jQuery);